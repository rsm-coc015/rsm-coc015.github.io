...
To gain a more intuitive understanding of how A/B testing works, we can use simulated data to observe behavioral differences between the two groups of users.

In this example, we assume that users who see CTA A are slightly more likely to register than those who see CTA B.
```{python}
import numpy as np
...
```
This code simulates 1,000 visitors for each group and calculates the sample conversion rates and the difference between the two groups. Although we set the true values to 0.12 and 0.10, the sample rates obtained from a single simulation typically do not exactly match the true values. This is the source of sampling variance and a core issue that statistical inference must address.


## The Law of Large Numbers

The results of a single simulation may be subject to random variation, so we repeat the experiment multiple times to observe the distribution of the estimates.
```{python}
diffs = []
...
plt.title("Distribution of Estimated Differences")
plt.xlabel("Difference (A - B)")
plt.ylabel("Frequency")
plt.show()
```
As can be seen from the plot, as the number of simulations increases, the estimated values gradually converge to the true difference, demonstrating the Law of Large Numbers.
